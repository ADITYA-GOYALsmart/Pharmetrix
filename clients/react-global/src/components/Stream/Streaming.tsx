...
const Streaming: React.FC = () => {
  ...
  // --------------------------------------------
  // API STUBS (NO REAL CALLS)
  // --------------------------------------------

  const connectWebSocket = (id: string) => {
    console.log(`connectWebSocket(${id}) called`);
    setStatus("Connected (mock)");
    // You can show a dummy frame:
    if (imgRef.current) {
      imgRef.current.src = "";
    }
  };
  ...
  const fetchStatus = (id: string) => {
    console.log(`fetchStatus(${id}) called`);
    // Mock: always inactive
    setLive(false);
  };

  const toggleFeed = () => {
    console.log(`toggleFeed() called for ${piId}, new state = ${!live}`);
    setLive(!live);
  };
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto" }}>
      <h1 style={{ marginBottom: "30px", color: "#283593" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        >
          ...
          >
            <span>
              {selectedPi ? selectedPi.name : "Select a Pi device..."}
            </span>
            <span
              style={{
              ...
              {availablePis.map((pi) => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor:
                      selectedPi?.id === pi.id ? "#f5f5f5" : "white",
                  }}
                >
                  <div
                    style={{
                    ...
                    <div>
                      <span
                        style={{
                          ...
                          fontWeight: "500",
                          backgroundColor:
                            pi.status === "online" ? "#d4edda" : "#f8d7da",
      ...
      >
        <div style={{ marginBottom: "8px" }}>
          ...
          <span style={{ color: "#155724" }}>{status}</span>
        </div>
        <div style={{ marginBottom: "16px" }}>
          <strong>Live Feed:</strong>{" "}
          <span style={{ color: live ? "#155724" : "#721c24" }}>
